...
class GLUBlock(nn.Module):
    """
    GLU block with optional layer normalization and dropout.
    
    This is a complete GLU block that can be used directly or wrapped
    with ResNetWrapper for residual connections.
    """
    ...
    def __call__(self, x: jnp.ndarray, training: bool = True) -> jnp.ndarray:
        ...
        Returns:
            GLU block output [batch_size, features]
        """
        # Apply GLU layer
        glu_layer = GLULayer(
        ...
        output = glu_layer(x, training=training)
...
def create_glu_block(features: int,
                    use_bias: bool = True,
                    use_layer_norm: bool = True,
                    dropout_rate: float = 0.0,
                    gate_activation: Callable = nn.sigmoid,
