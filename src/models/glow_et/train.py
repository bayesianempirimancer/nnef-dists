...
This script provides a command-line interface for training GLOW ET models.
...
Usage:
    python src/training/glow_et_trainer.py --data data/training_data.pkl --epochs 100
    python src/training/glow_et_trainer.py --data data/my_data.pkl --num-flow-layers 6 --features 128 64
"""
...
if __name__ == "__main__":
    ...
    from src.models.glow_et.model import Config, Glow_ET_Net
else:
    # When used as module, use relative imports
    from .model import Config, Glow_ET_Net
...
class TrainingConfig(BaseTrainingConfig):
    """
    Glow ET specific training configuration.
    
    This configuration inherits from BaseTrainingConfig and adds any Glow ET
    specific training parameters. Currently uses all base defaults.
    """
    
    # Glow ET specific training switches
    training_switches = [
...
def train_model(
    ...
    model = Glow_ET_Net(config=model_config)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Glow ET models with configurable architecture and training parameters",
    ...
    parser.add_argument("--features", type=int, nargs='+', default=None,
                        help="Feature layer sizes (default from config)")
    parser.add_argument("--num-flow-layers", type=int, default=None,
    ...
    print("\n" + "="*60)
    print("✅ SUCCESS! Glow ET training completed")
    """
    ...
    # Create model config with GLOW-specific parameters
    model_kwargs = {
        'input_dim': eta_dim,
        'output_dim': mu_dim
    }
    ...
    model_attributes = ['num_flow_layers', 'features', 'activation', 'use_residual', 'use_actnorm', 
                       'residual_weight', 'log_scale_clamp', 'translation_clamp', 'dropout_rate', 'initialization_method']
    ...
    model_config = Glow_Et_Config.create(**model_kwargs)
...
def train_model(model_config: Glow_ET_Config, training_config: BaseTrainingConfig, 
                data: Dict[str, Any], output_dir: Path,
                epochs: int, dropout_epochs: Optional[int]) -> Dict[str, Any]:
    """
    Train the GLOW ET model using the provided configurations and data.
    ...
    model = Glow_ET_Network(config=model_config)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = BaseETTrainer.create_base_argument_parser("GLOW ET Training Script")
    
    # Add model-specific arguments
    parser.add_argument("--num-flow-layers", type=int,
                       help="Number of flow layers (default from config)")
    parser.add_argument("--features", type=int, nargs="+",
                       help="Hidden layer sizes for coupling networks (default from config)")
    parser.add_argument("--activation", type=str, choices=["relu", "gelu", "swish", "tanh"],
                       help="Activation function (default from config)")
    parser.add_argument("--use-residual", action="store_true",
                       help="Use residual connections in coupling networks (default from config)")
    parser.add_argument("--use-actnorm", action="store_true",
                       help="Use activation normalization (default from config)")
    parser.add_argument("--residual-weight", type=float,
                       help="Weight for residual connections (default from config)")
    parser.add_argument("--log-scale-clamp", type=float, nargs=2, metavar=('MIN', 'MAX'),
                       help="Clamping range for log_scale (min max) (default from config)")
    parser.add_argument("--translation-clamp", type=float, nargs=2, metavar=('MIN', 'MAX'),
                       help="Clamping range for translation (min max) (default from config)")
...
def main():
    ...
    if args.output_dir:
        ...
        output_dir = Path(f"artifacts/glow_et_{timestamp}")
