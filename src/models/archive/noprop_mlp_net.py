...
@dataclass
class NoProp_MLP_Config(BaseConfig):
    """Configuration for NoProp MLP Network."""
    
    # Model type identification
    model_type: str = "noprop_mlp"
    model_name: str = "noprop_mlp_network"
    
    # Network architecture (inherits from BaseConfig)
    layer_norm_type: str = "weak_layer_norm"
    
    # Override time embedding defaults for NoProp MLP
    time_embed_dim: int = 20
    time_embed_method: str = "sinusoidal"
    
    # Embedding parameters (time embedding params inherited from BaseConfig)
    ...
    def _validate_model_specific(self) -> None:
        """Validate NoProp MLP specific parameters."""
        # Validate loss type
        valid_loss_types = ["noprop", "flow_matching"]
        if self.loss_type not in valid_loss_types:
            raise ValueError(f"loss_type must be one of {valid_loss_types}, got {self.loss_type}")
        
        # Validate time embedding method
        valid_time_embed_methods = ["sinusoidal", "fourier", "log_freq", "cyclical_fourier", "linear", "constant"]
        if self.time_embed_method not in valid_time_embed_methods:
            raise ValueError(f"time_embed_method must be one of {valid_time_embed_methods}, got {self.time_embed_method}")
    ...
    def get_architecture_summary(self) -> str:
        ...
        return f"NoProp MLP: {self.input_dim} -> {hidden_str} -> {self.output_dim} ({self.activation})"
    ...
    @classmethod
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'NoProp_MLP_Config':
        """Create configuration from dictionary."""
        return cls(**config_dict)
    
    def __str__(self) -> str:
        """String representation of configuration."""
        return f"NoProp_MLP_Config(type={self.model_type}, arch={self.get_architecture_summary()})"
    
    def __repr__(self) -> str:
        """Detailed string representation."""
        return (f"NoProp_MLP_Config("
...
def create_noprop_mlp_config(
    ...
    **kwargs
    ...
    Args:
        ...
        time_embed_method: Time embedding method
        eta_embed_dim: Eta embedding dimension
        eta_embedding_type: Type of eta embedding
    ...
    config = NoProp_MLP_Config(
        ...
        time_embed_method=time_embed_method,
...
@dataclass
class NoProp_MLP_FlowMatching_Config(BaseConfig):
    """Configuration for NoProp MLP Network with Flow Matching."""
    
    # Model type identification
    model_type: str = "noprop_mlp_flow_matching"
    model_name: str = "noprop_mlp_flow_matching_network"
    ...
    hidden_sizes: List[int] = field(default_factory=lambda: [32, 32, 32])
    activation: str = "swish"
    ...
    def _validate_model_specific(self) -> None:
        """Validate NoProp MLP Flow Matching specific parameters."""
        # Validate loss type
        if self.loss_type != "flow_matching":
            raise ValueError(f"loss_type must be 'flow_matching' for this config, got {self.loss_type}")
    ...
    def get_architecture_summary(self) -> str:
        """Get a summary of the model architecture."""
        hidden_str = " -> ".join(map(str, self.hidden_sizes))
        return f"Flow Matching MLP: {self.input_dim} -> {hidden_str} -> {self.output_dim} ({self.activation})"
    
    def to_dict(self) -> Dict[str, Any]:
        """Convert config to dictionary."""
        base_dict = super().to_dict()
        base_dict.update({
            'model_type': 'noprop_mlp_flow_matching',
    ...
    @classmethod
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'NoProp_MLP_FlowMatching_Config':
        """Create configuration from dictionary."""
        return cls(**config_dict)
    
    def __str__(self) -> str:
        """String representation of configuration."""
        return f"NoProp_MLP_FlowMatching_Config(type={self.model_type}, arch={self.get_architecture_summary()})"
    
    def __repr__(self) -> str:
        """Detailed string representation."""
        return (f"NoProp_MLP_FlowMatching_Config("
                f"model_type='{self.model_type}', "
                f"input_dim={self.input_dim}, "
                f"output_dim={self.output_dim}, "
                f"hidden_sizes={self.hidden_sizes}, "
...
def create_noprop_mlp_flow_matching_config(
    input_dim: int, 
    output_dim: int,
    ...
    n_time_steps: int = 20,
    **kwargs
) -> NoProp_MLP_FlowMatching_Config:
    """
    Create a NoProp MLP Flow Matching configuration with specified parameters.
    ...
    Args:
        ...
        loss_type: Loss type (fixed to "flow_matching")
        n_time_steps: Number of time steps for integration during prediction
        **kwargs: Additional parameters to override
        
    Returns:
        NoProp_MLP_FlowMatching_Config instance
    """
    config = NoProp_MLP_FlowMatching_Config(
        input_dim=input_dim,
        output_dim=output_dim,
        hidden_sizes=hidden_sizes,
        activation=activation,
