...
def create_cond_resnet(model_type: str, model_config: dict) -> nn.Module:
    ...
    Args:
        model_type: Type of model to create ("conditional_resnet_mlp", "geometric_flow", "potential_flow", "potential_flow_wrapper", "geometric_flow_wrapper")
        model_config: Dictionary containing model configuration parameters
        
    Returns:
        Instantiated model
    """
    if model_type == "ConditionalResnet_MLP":
        return ConditionalResnet_MLP(**model_config)
    elif model_type == "NaturalFlow":
        return NaturalFlow(**model_config)
    elif model_type == "GeometricFlow":
        return GeometricFlow(**model_config)
    elif model_type == "PotentialFlow":
...
class NaturalFlow(nn.Module):
    """
    dz/dt parameterized to be consistent with a Natural Flow, i.e. dmu_T/dt = Sigma(\\eta)\\eta
    where Sigma is parameterized by a conditional ResNet.
...
class PotentialFlow(nn.Module):
    ...
    @nn.compact
    def __call__(self, z: jnp.ndarray, x: jnp.ndarray, t: jnp.ndarray, training: bool = True) -> jnp.ndarray:
        """Compute the gradient of the potential function with respect to z.
        ...
        Returns:
            Gradient of potential w.r.t. z [batch_size, z_dim]
        """        
        # Convert string activation function to callable
        activation_fn = get_activation_function(self.activation_fn)
        ...
        grad_fn = jax.grad(potential_fn, argnums=0)  # gradient w.r.t. first argument (z)
        vmapped_grad_fn = jax.vmap(grad_fn, in_axes=(0, 0, 0))  # vmap over batch dimension
        
        # Get both gradient and value
        gradients = vmapped_grad_fn(z, x, t)
        
        # Return only the gradient (negative of gradient for potential flow)
        return -gradients
...
class GeometricFlow(nn.Module):
    ...
    def __call__(self, z: jnp.ndarray, x: jnp.ndarray, t: jnp.ndarray, training: bool = True) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        # Compute both gradient and value of potential with respect to z
        # Use vmap to ensure proper batching through the gradient computation
        hessian_fn = jax.hessian(potential_fn, argnums=0)  # Hessian w.r.t. first argument (z)
        
        # Compute the Hessian matrix for each batch element
        Sigma = jax.vmap(hessian_fn)(z, x, t)
        
        # Return gradient first, then value (as tuple)
        return jnp.einsum("...ij, ...j -> ...i", Sigma, x)
