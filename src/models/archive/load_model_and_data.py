...
def load_model_and_data(model_dir: str, data_file: Optional[str] = None) -> Tuple[Dict, Dict, Dict, Any, Any, Dict]:
    ...
    # Extract model type from config (support both old and new field names)
    model_type = config.get('model_type') or config.get('model_name')
    if model_type is None:
        raise ValueError("Config file must contain 'model_type' or 'model_name' field")
...
def make_predictions(model, params, eta_data: np.ndarray, batch_size: int = 100) -> jnp.ndarray:
    ...
    for i in range(0, len(eta_data), batch_size):
        ...
        # Check if this is a NoProp model that needs n_time_steps parameter
        if hasattr(model, 'config') and hasattr(model.config, 'loss_type') and model.config.loss_type in ['noprop', 'flow_matching']:
            # Use the config value for n_time_steps
