...
class NoPropTrainer:
    """
    Trainer for NoProp CT models using the NoProp training protocol.
    ...
    def __init__(self, model: NoPropCT, config: Config, training_config: BaseTrainingConfig = None):
        self.model = model
        self.config = config
    ...
    def train(self, 
        ...
        for epoch in range(num_epochs):
            ...
            def loss_func(params):
                # For CT model, use compute_loss method with (x, target, key) signature
...
def create_configs_from_args(args, eta_dim: int, mu_dim: int) -> Tuple[Config, BaseTrainingConfig]:
    ...
    if hasattr(args, 'noise_schedule'):
        config_kwargs['noise_schedule'] = args.noise_schedule
...
def train_model(model_config: Config, training_config: BaseTrainingConfig, 
    ...
    # Create model with specified noise schedule
    
    # Select noise schedule based on config
    if model_config.noise_schedule == "linear":
        noise_schedule = LinearNoiseSchedule()
    elif model_config.noise_schedule == "cosine":
        noise_schedule = CosineNoiseSchedule()
    elif model_config.noise_schedule == "sigmoid":
        noise_schedule = SigmoidNoiseSchedule()
    elif model_config.noise_schedule == "simple_learnable":
        noise_schedule = SimpleLearnableNoiseSchedule()
    elif model_config.noise_schedule == "learnable":
        noise_schedule = LearnableNoiseSchedule()
    else:
        noise_schedule = LinearNoiseSchedule()  # Default fallback
    
    model = NoPropCT(config=model_config, noise_schedule=noise_schedule)
    trainer = NoPropTrainer(model, model_config, training_config)
...
def add_noprop_mlp_arguments(parser):
    """Add NoProp CT specific arguments to parser."""
