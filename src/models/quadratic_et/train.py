...
if __name__ == "__main__":
    ...
    from src.models.quadratic_et.model import Config, Quadratic_ET_Net
else:
    # When used as module, use relative imports
    from .model import Config, Quadratic_ET_Net
...
class TrainingConfig(BaseTrainingConfig):
    """
    Quadratic ET specific training configuration.
    
    This configuration inherits from BaseTrainingConfig and adds any Quadratic ET
    specific training parameters. Currently uses all base defaults.
    """
    
    # Quadratic ET specific training switches
    training_switches = [
...
def train_model(
    ...
    model = Quadratic_ET_Net(config=model_config)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Quadratic ET models with configurable architecture and training parameters",
    ...
    parser.add_argument("--use-activation", action="store_true",
                        help="Use activation function (default from config)")
    ...
    if 'hidden_sizes' not in model_kwargs:
        model_kwargs['hidden_sizes'] = (16,)      # Class default, now a tuple
    elif isinstance(model_kwargs['hidden_sizes'], list):  # Convert list to tuple
        model_kwargs['hidden_sizes'] = tuple(model_kwargs['hidden_sizes'])
    ...
    model_config = Config.create(**model_kwargs)
