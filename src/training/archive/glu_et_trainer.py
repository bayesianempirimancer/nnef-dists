"""
Training implementation for GLU ET models.

This module implements a Hugging Face compatible training protocol for
GLU-based ET networks that directly predict expected sufficient statistics
from natural parameters.
"""

from typing import Dict, Any, Optional, Tuple, List
import jax
import jax.numpy as jnp

from .base_et_trainer import BaseETTrainer
from ..models.glu_et_net import GLU_ET_Network
from ..configs.glu_et_config import GLU_ET_Config


class GLUETTrainer(BaseETTrainer):
    """
    Trainer for GLU ET models using standard backpropagation.
    
    This implements a standard training loop with:
    - Standard backpropagation
    - MSE loss for ET prediction
    - Optional regularization (weight decay, dropout)
    - Standard optimization (Adam, etc.)
    """
    

