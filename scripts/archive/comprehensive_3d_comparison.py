#!/usr/bin/env python3
"""
Comprehensive comparison of ALL neural network approaches on 3D Gaussian dataset.

This script tests all implemented approaches:
1. Standard MLP (baseline)
2. NoProp-CT (continuous-time)
3. Diffusion Model
4. Improved INN
5. Division-Aware MLP
6. GLU-based MLPs
7. Quadratic ResNet
8. Transformer Networks
9. Deep Flow Network (NEW!)
...
from src.glu_moment_net import GLUMomentNet, DeepGLUMomentNet, create_glu_train_state, train_glu_moment_net
from src.quadratic_resnet import QuadraticResNet, DeepQuadraticResNet, AdaptiveQuadraticResNet, create_quadratic_train_state, train_quadratic_moment_net
from src.transformer_moments import TransformerMomentNet, QuadraticTransformerMomentNet, create_transformer_train_state, train_transformer_moment_net
...
def compute_ground_truth_3d(eta: jnp.ndarray) -> jnp.ndarray:
    ...
    # Flatten to match our data format: [μ₁, μ₂, μ₃, Σ₁₁, Σ₁₂, Σ₁₃, Σ₂₂, Σ₂₃, Σ₃₃]
...
def train_glu(train_data, val_data, test_data, config):
    """Train GLU-based model."""
    ...
    params, optimizer, opt_state = create_glu_train_state(model, random.PRNGKey(46), test_data['eta'][:1])
    
    history = train_glu_moment_net(
...
def train_quadratic_resnet(train_data, val_data, test_data, config):
    """Train adaptive quadratic ResNet."""
    
    model = AdaptiveQuadraticResNet(
        hidden_size=config['hidden_size'],
        num_layers=config['num_layers'],
        learning_rate=config['learning_rate']
    )
    
    params, optimizer, opt_state = create_quadratic_train_state(model, random.PRNGKey(47), test_data['eta'][:1])
    
    history = train_quadratic_moment_net(
        model, params, optimizer, opt_state, train_data, val_data, config
    )
...
def train_transformer(train_data, val_data, test_data, config):
    """Train quadratic transformer."""
    ...
    params, optimizer, opt_state = create_transformer_train_state(model, random.PRNGKey(48), test_data['eta'][:1], model_type='quadratic_transformer')
    
    history = train_transformer_moment_net(
        model, params, optimizer, opt_state, train_data, val_data, config, model_type='quadratic_transformer'
    )
...
def create_comparison_plots(results: Dict[str, Dict], output_dir: Path):
    ...
    successful_results = {k: v for k, v in results.items() if v['status'] != 'failed'}
    ...
    plt.savefig(output_dir / "comprehensive_3d_comparison.png", dpi=150, bbox_inches='tight')
    plt.savefig(output_dir / "comprehensive_3d_comparison.pdf", bbox_inches='tight')
    plt.close()
...
def main():
    """Run comprehensive comparison on 3D Gaussian dataset."""
    
    print("🚀 COMPREHENSIVE 3D GAUSSIAN COMPARISON")
    print("=" * 80)
    print("Testing ALL neural network approaches with ground truth comparison")
    ...
    models_to_test = {
        ...
        'GLU MLP': train_glu,
        'Quadratic ResNet': train_quadratic_resnet,
        'Quadratic Transformer': train_transformer,
    ...
    output_dir = Path("artifacts/comprehensive_3d_comparison")
    ...
    print(f"\n✅ Comprehensive 3D comparison completed!")
